/*
	...
	using various payload types (from PayloadTypeFactory.java) containing an operating system command (from PayloadCommandFactory.java) to be executed on a target 
	...
	plaintext in the message body (JBossSerializedInsertionPoint.java for JBoss target systems), or b) the payload will be base64-encoded and inserted into the 
	appropriate request parameter(s) (WebSphereSerializedInsertionPoint.java for WebSphere target systems). The operating system command included in the payload 
	will force the target system to (via HTTP) a) access the SuperSerial Node, or b) upload a file to the SuperSerial Node. After submitting the payload to the target 
	system, the Active Scanner will attempt to a) read the access entry from the SuperSerial node, or b) download the file that was uploaded by the target system. If 
	this is successful, a new "Java Deserialization Vulnerability" Scan Issue (SerializationRCEExploitIssue.java) will be created, displaying the request containing 
	the successful payload, the SuperSerial Node response containing the access entry or uploaded file, and details about the detection of the vulnerability and about 
	the target system (web server platform and operating system).
...
public class BurpExtender implements IBurpExtender,IScannerCheck {
	...
	private static final String disclaimer = "\nDisclaimer: This software is only intended to be used against systems the user explicitly owns or has authorization to "+
												"test/attack. The developers provide the software for free without warranty, and assume no responsibility for any damage"+
												" caused to systems by misusing the software. It is the responsibility of the user to abide by all local, state and "+
												"federal laws while using the software."; //TODO: move disclaimer to text file for easier future editing
	private static final int INTERNAL_ERROR_CODE = 70; //ysoserial internal error code
	private static final int USAGE_CODE = 64; //ysoserial "printUsage()" code
	...
	public List<IScanIssue> doActiveScan(IHttpRequestResponse baseRequestResponse,IScannerInsertionPoint insertionPoint) {
		...
		if(ipName.equals(JBOSS_INSERTION_POINT) || ipName.equals(WEBSPHERE_INSERTION_POINT)) { //insertion point is SuperSerial custom insertion point
			...
			if(respInfo!=null && respInfo.getStatusCode()==200) { //queue request completed successfully
				...
				if(nodePath==null) { //if Node does not return a context to use (maybe not a node afterall?), issue error alert and abort active scan; TODO: improve error verbosity
					callbacks.printError("Active Scans check skipped on "+urlStr+" due to invalid Node connection settings! Set settings in the "+configTab.getTabCaption()+"->\"Node Connection Settings\" tab and click \"Test Connection\"!");
					callbacks.issueAlert("Invalid SuperSerial Node connection settings! Active Scans check will be skipped! Set settings in the "+configTab.getTabCaption()+"->\"Node Connection Settings\" tab and click \"Test Connection\"!");
					return issues;
				}
				...
				if(payloadTypes.length==0) {
					callbacks.printError("Active Scans check skipped on "+urlStr+" due to invalid scan settings (no payload types enabled)! Enable at least one payload type in the "+configTab.getTabCaption()+"->\"Scan Settings\" tab!");
					callbacks.issueAlert("Invalid SuperSerial scan settings: no payload types enabled! Active scan checks will be skipped! Enable at least one payload type in the "+configTab.getTabCaption()+"->\"Scan Settings\" tab!");
					return issues;
				}
				...
				for(i=0;!nodeConnError && i<payloadTypes.length;i++) { //loop through payload types
					...
					for(j=0;!nodeConnError && j<commands.length;j++) { //loop through commands, stop if node access/file upload is detected
						...
						try {
							...
							System.setOut(ps);
						} catch(Exception e) { //error allocating resources for generating payload, print exception to errors tab and skip to next payload type
							e.printStackTrace();
							continue;
						}
						
						//change stderr from System.err to Burp stderr OutputStream, ignore any errors if fails
						...
						try {
							...
							switch(ee.getStatus()) { 
								case 0: //normal ysoserial execution
									break;
								default: //error ysoserial execution (internal error or usage error)
									callbacks.printError("Error occurred in ysoserial ("+(ee.getStatus()==INTERNAL_ERROR_CODE ? "internal" : "usage")+" error) generating "+technique+" payload with command: "+cmd);
									callbacks.issueAlert("Error occurred in ysoserial ("+(ee.getStatus()==INTERNAL_ERROR_CODE ? "internal" : "usage")+" error) generating "+technique+" payload with command: "+cmd);
									System.setSecurityManager(null);
						...
						//get payload byte array and reset stdout to System.out
						...
						//Set stdout back to System.out and stderr back to System.err
