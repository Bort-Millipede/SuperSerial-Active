/*
	...
	SuperSerial-Active Scan check: Conduct active scan check that involves sending a request including a Java serialized object generated by ysoserial containing 
	an operating system command (from PayloadCommandFactory.java) to be executed on a target system. The base request will either be a) converted to a POST request 
	containing the exploit Java serialized object in the message body (JBossSerializedInsertionPoint.java for JBoss target systems), or b) base64-encoded and added 
	to an appropriate parameter in the base request (WebSphereSerializedInsertionPoint.java for WebSphere target systems). Command will force target system to (via 
	HTTP) a) access a SuperSerial Node, or b) upload a file to a SuperSerial node. After submitting payload to target system, the active scanner will attempt to 
	a) read the access entry from the SuperSerial node, or b) download the file that was uploaded by the target system. If this is successful, a new "Java 
	Deserialization Vulnerability" (SerializationRCEExploitIssue.java) Scan Issue will be created, displaying the request containing the exploit Java serialized 
	object, the SuperSerial node response containing the access entry or uploaded file, and details about the detection of the vulnerability and about the target 
	system (web server platform and operating system).
...
public class BurpExtender implements IBurpExtender,ITab,IScannerCheck {
	...
	public void registerExtenderCallbacks(IBurpExtenderCallbacks mCallbacks) {
		...
		callbacks.printOutput("Node settings uninitialized, set in "+caption+" tab and click \"Test Connection\"");
