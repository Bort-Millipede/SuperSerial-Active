...
public class SuperSerialNodeHttpHandler implements HttpHandler {
	...
	public void handle(HttpExchange exchange) {
		...
		try {
			...
			//System.err.println(method+" request received by "+path+" from "+exchange.getRemoteAddress().getHostString());
			...
			if(token.equalsIgnoreCase(clientToken)) {
				...
				switch(method) {
					case "PUT": //upload a file
						...
						if((tempFile == null) && (!written)) { //if no file has been already uploaded, read file from request
							...
							tempFile = File.createTempFile("SuperSerial","upload");
							...
							System.err.println("file (size: "+fileArray.length+" bytes) uploaded successfully to "+path+" by "+source);
							...
							jsonStr = retJson.toString();
							System.err.println(method+" request from "+exchange.getRemoteAddress().getHostString()+" to "+path+" denied (file already uploaded)");
							Headers respHeaders = exchange.getResponseHeaders();
							respHeaders.add("Content-Type","application/json");
					...
					case "GET": //retrieve uploaded file
						...
						if(!write) { //if request is to read uploaded file
							if((tempFile != null) && (written)) { //if a file is ready for reading
								...
								os.close();
								if(uploaded) {
									System.err.println("File (size: "+fileArray.length+" bytes) downloaded successfully from "+path+" by "+source);
								} else {
									System.err.println("Access entry downloaded successfully from "+path+" by "+source);
								}
								...
								jsonStr = retJson.toString();
								System.err.println(method+" request from "+exchange.getRemoteAddress().getHostString()+" to "+path+" denied (no file to download)");
								Headers respHeaders = exchange.getResponseHeaders();
								respHeaders.add("Content-Type","application/json");
								exchange.sendResponseHeaders(500,jsonStr.length());
								os = exchange.getResponseBody();
							...
							if((tempFile == null) && (!written)) { //if no file has been already uploaded
								source = exchange.getRemoteAddress().getHostString();
								time = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss").format(currTime);
								tempFile = File.createTempFile("SuperSerial",null);
								FileOutputStream fos = new FileOutputStream(tempFile);
								...
								fos.close();
								written = true;
								System.err.println("Access entry successfully written to "+path+" by "+source);
								...
								retJson.put("status","Failure: file already uploaded");
								jsonStr = retJson.toString();
								System.err.println(method+"request from "+exchange.getRemoteAddress().getHostString()+" to "+path+" denied (file already uploaded or access entry already written)");
								Headers respHeaders = exchange.getResponseHeaders();
								respHeaders.add("Content-Type","application/json");
								exchange.sendResponseHeaders(500,jsonStr.length());
								os = exchange.getResponseBody();
					...
					default: //unsupported method: reject request with HTTP 405 response
						...
						//exchange.getResponseBody().close();
						System.err.println(method+" request from "+exchange.getRemoteAddress().getHostString()+" to "+path+" denied (unsupported HTTP method: "+method+")");
						break;
				}
			} else {
				exchange.getRequestBody().close();
				...
				exchange.getResponseBody().close();
				System.err.println(method+" request from "+exchange.getRemoteAddress().getHostString()+" to "+path+" denied (wrong authentication token)");
			}
